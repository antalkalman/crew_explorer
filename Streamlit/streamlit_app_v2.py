...
filtered_departments = sorted(df["Department"].dropna().unique())

# Initialize session state
if "current_dept_index" not in st.session_state:
    st.session_state.current_dept_index = 0
if "filtered_departments" not in st.session_state:
    st.session_state.filtered_departments = filtered_departments
...
# Prepare unique list of departments
filtered_departments = sorted(df["Department"].dropna().unique())
st.session_state.filtered_departments = filtered_departments
...
with col1:
    if st.button("⬅️ Previous"):
        st.session_state.current_dept_index = max(0, st.session_state.current_dept_index - 1)
with col2:
    if st.button("Next ➡️"):
        st.session_state.current_dept_index = min(len(filtered_departments) - 1, st.session_state.current_dept_index + 1)



# Override department filter with current one
selected_dept = filtered_departments[st.session_state.current_dept_index]
